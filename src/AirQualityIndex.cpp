/*
	HIL_LANGUAGE_COUNT.cpp - Library for getting weather related information.
*/
...
//    return Td;
    return !isFahrenheit ? Td : getCelsius(Td);
}
...
String Weather::getHeatIndexTag(float heatIndex) {
    NWS_HEAT_INDEX_LEVEL level;

    if (79 >= heatIndex) {
        // Normal, less than 80
        level = NWS_HIL_NORMAL;
    } else if ((80 <= heatIndex) && (90 > heatIndex)) {
        // Caution, 80 ~ 89
        level = NWS_HIL_CAUTION;
    } else if ((90 <= heatIndex) && (106 > heatIndex)) {
        // Extream Caution, 90 ~ 105
        level = NWS_HIL_EXT_CAUTION;
    } else if ((106 <= heatIndex) && (130 > heatIndex)) {
        // Danger, 106 ~ 129
        level = NWS_HIL_DANGER;
    } else if (130 <= heatIndex) {
        // Extream Danger, over 130
        level = NWS_HIL_EXT_DANGER;
    }

    return m_hilDesc[level].lang[m_language].levelTag
}

String Weather::getHeatIndexDesc(float heatIndex) {
    NWS_HEAT_INDEX_LEVEL level;
    ...
    return m_hilDesc[level].lang[m_language].levelDesc
}
